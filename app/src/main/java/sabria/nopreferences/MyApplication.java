...
import sabria.noperferences.library.core.NoPreferences;
...
public class MyApplication extends Application {
    ...
    private void initSpProxy() {
        proxy = NoPreferences.create(application, PersonProxy.class);
    }

    public static PersonProxy getProxy(){
        if(proxy==null){
            Toast.makeText(application, "proxy==null", Toast.LENGTH_LONG).show();
            proxy = new NoPreferences().create(application, PersonProxy.class);
        }
